...
当前 StoreV1 没有完成回调接口，实现采用有界扫描（默认最多128个活跃 shard，可配置）。
...
析构会等待所有已接收任务结束，再停止 ready 线程。活跃名额默认128，可配置为1..8192；待处理工作的上限仍为8192。
...
以下配置放在 `ucm_connectors[].ucm_connector_config` 下，修改后重启全部 TP worker。

| 配置 | 默认值 | 说明 |
|---|---:|---|
| `compress_ready_poll_us` | 10 | 范围0..1000000微秒；0表示有未完成读取时连续扫描，不做sleep/yield；没有在途读取时仍休眠 |
| `compress_max_active_loads` | 128 | 范围1..8192；计入读取中、等待解码及解码中的shard，不是解码线程数 |

固定 R160、TP4、layer-wise、24个解码线程、8000输入/1输出、并发16、90%前缀命中、原POSIX引擎和O_DIRECT设置：

| 实验 | ready_poll_us | max_active_loads | 用途 |
|---|---:|---:|---|
| C0 | 10 | 128 | 复现当前C |
| C1 | 0 | 128 | 仅取消轮询等待 |
| C2 | 10 | 24 | 仅缩小活跃窗口 |
...
```yaml
# 在已有 ucm_connector_config 中增加；此处为 C1
compress_ready_poll_us: 0
compress_max_active_loads: 128
decompress_thread_num: 24
```

编译部署一次后只改配置。启动日志必须出现 `ready_poll_us=...` 和 `max_active_loads=...`，
并确认4个TP worker都生效。C0/C1/C2使用相同数据、预热及Host缓存状态控制流程，每组至少3轮，
交错运行并比较TTFT中位数，同时记录CPU占用和实际读盘量；不能让后跑的组仅因Host缓存命中变多而占优。
忙轮询可占用每个Compress实例约一个CPU核（TP4通常有4个实例），必须记录CPU资源竞争。
若C1改善，完成发现/等待策略影响性能；若C2改善，活跃窗口影响关键路径；都不改善则继续定位
队列交接、逐层就绪及H2D时间，不能据此直接认定某一个内部阶段是根因。
